...
# Generated: Wed Apr 17 19:08:46 2013
##################################################
...
class simple_qam(gr.top_block, Qt.QWidget):
	...
	def __init__(self):
		...
		self.sinc_pulse = sinc_pulse = gr.firdes.low_pass(1, 1, 0.5/sps, 1./10000)
		...
		self.filt_energy = filt_energy = sum(abs(array(sinc_pulse))**2)
		self.constellation_type = constellation_type = 'qam' # 'pam', 'qam' or 'psk'
		self.snr_db = snr_db = 15
		self.pulse = pulse = array(sinc_pulse)/sqrt(filt_energy)
		...
		self.first_symbol_delay = first_symbol_delay = len(pulse)-1
		...
		self.tabid_1_widget_3 = Qt.QWidget()
		self.tabid_1_layout_3 = Qt.QBoxLayout(Qt.QBoxLayout.TopToBottom, self.tabid_1_widget_3)
		self.tabid_1_grid_layout_3 = Qt.QGridLayout()
		self.tabid_1_layout_3.addLayout(self.tabid_1_grid_layout_3)
		self.tabid_1.addTab(self.tabid_1_widget_3, "signal-based")
		...
		self.tabid_1_layout_3.addWidget(self._qtgui_sink_x_0_2_win)
		...
		self.interp_fir_filter_xxx_0 = filter.interp_fir_filter_ccf(sps, (pulse))
		...
		self.gr_skiphead_0 = gr.skiphead(gr.sizeof_gr_complex*1, sps-1)
		...
		self.gr_noise_source_x_0 = gr.noise_source_c(gr.GR_GAUSSIAN, 0, 0)
		self.gr_nlog10_ff_0 = gr.nlog10_ff(1, 1, 0)
		self.gr_multiply_const_vxx_1_0 = gr.multiply_const_vcc((1./sqrt(sps), ))
		self.gr_multiply_const_vxx_1 = gr.multiply_const_vcc((sqrt(sps), ))
		...
		self.gr_fir_filter_xxx_0 = gr.fir_filter_ccf(sps, ([1.0*i for i in pulse]))
		self.gr_file_sink_0_1_1 = gr.file_sink(gr.sizeof_gr_complex*1, "tx_sig.32fc")
		self.gr_file_sink_0_1_1.set_unbuffered(False)
		self.gr_file_sink_0_1_0_0 = gr.file_sink(gr.sizeof_gr_complex*1, "rx_sig.32fc")
		self.gr_file_sink_0_1_0_0.set_unbuffered(False)
		...
		self.connect((self.gr_fir_filter_xxx_0, 0), (self.gr_multiply_const_vxx_0, 0))
		self.connect((self.gr_fir_filter_xxx_0, 0), (self.qtgui_sink_x_0_0_0_0, 0))
		...
		self.connect((self.gr_skiphead_0, 0), (self.gr_fir_filter_xxx_0, 0))
		self.connect((self.digital_chunks_to_symbols_xx_0, 0), (self.gr_multiply_const_vxx_0_0, 0))
		self.connect((self.gr_multiply_const_vxx_0_0, 0), (self.interp_fir_filter_xxx_0, 0))
		...
		self.connect((self.interp_fir_filter_xxx_0, 0), (self.gr_multiply_const_vxx_1, 0))
		self.connect((self.gr_multiply_const_vxx_1, 0), (self.gr_add_xx_0, 0))
		...
		self.connect((self.gr_multiply_const_vxx_1_0, 0), (self.gr_skiphead_0, 0))
		self.connect((self.gr_throttle_0, 0), (self.gr_multiply_const_vxx_1_0, 0))
		self.connect((self.gr_multiply_const_vxx_1, 0), (self.gr_file_sink_0_1_1, 0))
		self.connect((self.gr_multiply_const_vxx_1, 0), (self.qtgui_sink_x_0_2, 0))
		self.connect((self.gr_multiply_const_vxx_1_0, 0), (self.gr_file_sink_0_1_0_0, 0))
		self.connect((self.gr_multiply_const_vxx_0, 0), (self.digital_constellation_receiver_cb_0, 0))
		self.connect((self.gr_multiply_const_vxx_0, 0), (self.gr_file_sink_0_1_0, 0))
	...
	def set_sps(self, sps):
		...
		self.gr_multiply_const_vxx_1.set_k((sqrt(self.sps), ))
		self.gr_multiply_const_vxx_1_0.set_k((1./sqrt(self.sps), ))
		self.set_sinc_pulse(gr.firdes.low_pass(1, 1, 0.5/self.sps, 1./10000))
	...
	def get_sinc_pulse(self):
		return self.sinc_pulse

	def set_sinc_pulse(self, sinc_pulse):
		self.sinc_pulse = sinc_pulse
		self.set_filt_energy(sum(abs(array(self.sinc_pulse))**2))
		self.set_pulse(array(self.sinc_pulse)/sqrt(self.filt_energy))
	...
	def get_filt_energy(self):
		return self.filt_energy

	def set_filt_energy(self, filt_energy):
		self.filt_energy = filt_energy
		self.set_pulse(array(self.sinc_pulse)/sqrt(self.filt_energy))
	...
	def set_pulse(self, pulse):
		...
		self.set_first_symbol_delay(len(self.pulse)-1)
		self.gr_fir_filter_xxx_0.set_taps(([1.0*i for i in self.pulse]))
	...
	def get_first_symbol_delay(self):
		return self.first_symbol_delay

	def set_first_symbol_delay(self, first_symbol_delay):
		self.first_symbol_delay = first_symbol_delay
